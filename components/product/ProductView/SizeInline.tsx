...
export default function SizeInline({
   ...
   currentAttribute = product?.stockCode?.substring(product?.stockCode?.lastIndexOf("-") + 1);
   const { openNotifyUser, closeNotifyUser,basketId,setCartItems,user } = useUI()
   ...
   console.log('MATCH_STRING',product?.stockCode?.substring(product?.stockCode?.lastIndexOf("-") + 1))
   ...
   let outerWearCategory = product?.mappedCategories?.find((x: any) => matchStrings(x.categoryName, "Outerwear", true));
   let innerWearCategory = product?.mappedCategories?.find((x: any) => matchStrings(x.categoryName, "Innerwear", true));
   const attrGroup = groupBy(product?.customAttributes, 'key')
   return (
      <>
         <div className='grid w-full grid-cols-3 px-4 border-gray-200 border-solid hide-swach-padding sm:border-t sm:px-0' id="productSize">
            <div className='col-span-2 item-left'>
               <h3 className="pt-3 text-xs font-semibold text-left text-gray-400 uppercase sm:text-sm">{label}:<span className="pl-1 text-xs font-bold text-black sm:text-sm">
            ...
            <div className='col-span-1 mt-3 text-right item-right'>
               <h3 className='text-sm font-semibold text-orange-600 cursor-pointer sm:text-md hover:text-orange-700' onClick={() => setSizeChart(true)}>Size Chart</h3>
            </div>
         </div>
         <RadioGroup value={'ring-gray-700'} onChange={() => { }} className="px-4 mt-4 mb-6 sm:px-0 hide-swach-padding">
            <RadioGroup.Label className="sr-only">{label}</RadioGroup.Label>
            <div className="flex items-center space-x-2">
               {items?.sort((a: any, b: any) => (a.displayOrder - b.displayOrder))?.map((item: any, idx: any) => {
                     ...
                     >
                        <>
                           <div className={classNames(
                              ...
                              `${(matchStrings(product?.stockCode?.substring(product?.stockCode?.lastIndexOf("-") + 1), sanitizeSize(item?.fieldValue), true) && stockAmount > 0) ? "border-gray-800" : soldoutCls}`,
                              ...
                              <Link href="javascript:void(0);">
                                 <>
                                    ...
                                    >
                                       {item.fieldValue
                                          ? generateItemOption(item.fieldValue, stockAmount)?.split('-')[0]
                                          : null}
                                    </a>
                                    {
                                       matchStrings(product?.stockCode?.substring(product?.stockCode?.lastIndexOf("-") + 1),item.fieldValue,true) && (
                                          stockAmount <= 5 &&
                                          <div
                                             className={classNames(
                                                selected.currentAttribute == generateItemOption(item?.fieldValue, stockAmount)?.toLowerCase()
                                                   ? 'inline-block top-14' : '', 'absolute block w-14 text-xs font-semibold text-dark-red mt-20'
                                             )
                                             }>
                                             {item.fieldValue
                                                ? generateItemOption(item.fieldValue, stockAmount)?.split('-')[1]
                                                : null}
                                          </div>
                           ...
                           <div className={classNames(
                              selected.currentAttribute == generateItemOption(item?.fieldValue, stockAmount)?.toLowerCase()
                                 ? 'inline-block top-14' : 'hidden', 'text-center absolute'
                           )
                           }>
                              <span className="absolute block mt-1 text-xs font-semibold text-red-500 w-14 ">{product?.currentStock} Left</span>
                           </div>
                           <div
                              className={classNames(
                                 selected.currentAttribute == generateItemOption(item?.fieldValue, stockAmount)?.toLowerCase()
                                    ? 'inline-block top-14' : 'hidden', 'absolute block w-14 mt-1 text-xs font-semibold text-red-500'
                              )
                              }>
                              {item.fieldValue
                                 ? generateItemOption(item.fieldValue, stockAmount)?.split('-')[1]
         ...
         <Transition.Root show={isSizeChart} as={Fragment}>
            <Dialog as="div" className="relative z-9999" onClose={() => setSizeChart(false)}>
                                    ...
                                    <div className="flex pb-2 border-b border-gray-200">
                                       ...
                                       >
                                          <div className="inline-block px-6">
                                             ...
                                             <XIcon className="relative w-6 h-6 text-zinc-600 -top-1" aria-hidden="true" />
                                          </div>
                                          <div className="inline-block text-left">
                                             <span className="flex-1 text-lg font-bold text-black">Size Chart</span>
                                             <span className='flex-1 block text-sm text-gray-500'>{product.name}</span>
                                          </div>
                                       </button>
                                    </div>
                                 </div>
                                             ...
                                             {isSizeChange ? (<>
                                                ...
                                                <div className="item-right">
                                                   <Switch
                                                      checked={enabled}
                                                      onChange={() => setEnable()}
                                                      className={`${enabled ? 'bg-white' : 'bg-white'} relative inline-flex switch-panel-parent transition-colors duration-200 dark:text-black ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75 switch-cm-before`}
                                                   >
                                                      <span className="sr-only">Use setting</span>
                                                      <span
                                                         aria-hidden="true"
                                             ...
                                             {isSizeChange ? (<>
                                                ...
                                                <div className="item-right">
                                                   <Switch
                                                      checked={enabled}
                                                      onChange={() => setEnable()}
                                                      className={`${enabled ? 'bg-white' : 'bg-white'} relative inline-flex dark:text-black switch-panel-parent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75 switch-cm-before`}
                                                   >
                                                      <span className="sr-only">Use setting</span>
                                                      <span
                                                         aria-hidden="true"
